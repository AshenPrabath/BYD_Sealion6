...
// ══  MAIN COMPONENT  ════════════════
// ══════════════════════════════════════
export default function Car360ViewerLite() {
              ...
              <div className="flex flex-col items-center gap-3">
                <div className="relative w-14 h-14">
                  <svg viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full drop-shadow-lg">
                    ...
                    <path d="M10 28 L4 28 M4 28 L8 24 M4 28 L8 32" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ animation: 'arrowLeft 1.2s ease-in-out infinite' }} />
                    <path d="M46 28 L52 28 M52 28 L48 24 M52 28 L48 32" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ animation: 'arrowRight 1.2s ease-in-out infinite' }} />
                  </svg>
                </div>
          ...
          <div className="w-full h-full cursor-grab active:cursor-grabbing relative">
            {!interiorReady && (
              <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-white/80 backdrop-blur-sm">
                <Loader2 className="w-8 h-8 animate-spin text-black/40 mb-4" />
                <div className="w-48 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                  <div className="h-full bg-black rounded-full transition-all duration-300 ease-out" style={{ width: `${interiorLoadProgress}%` }} />
                </div>
                <span className="text-[10px] text-black/40 font-bold uppercase tracking-widest mt-3">Loading Interior — {interiorLoadProgress}%</span>
              </div>
            )}
            {interiorReady && currentInteriorUrl && (
              <Canvas camera={{ position: [0, 0, 0.1], fov: 90 }} gl={{ toneMapping: THREE.NoToneMapping, outputColorSpace: THREE.SRGBColorSpace }}>
                {gyroActive ? <GyroControls orientationRef={orientationRef} /> : <OrbitControls enableZoom={false} enablePan={false} rotateSpeed={-0.5} />}
                <Suspense fallback={null}>
                  <InteriorSphere url={currentInteriorUrl} offsetX={interiorPosition === 'driver' ? 0.5 : 0} />
                </Suspense>
              </Canvas>
            )}
            ...
            <button
              ...
              className={`absolute top-4 left-4 z-50 p-2 rounded-full border backdrop-blur-md shadow-lg transition-all ${gyroActive ? 'bg-black text-white border-black' : 'bg-white/40 text-black border-white/20 hover:bg-white/60'}`}
            >
            ...
            <div className="absolute bottom-6 md:bottom-8 left-1/2 -translate-x-1/2 z-30 flex flex-col items-center space-y-3 md:space-y-4 pointer-events-auto">
              <div className="flex bg-white/30 backdrop-blur-md border border-white/20 rounded-full p-1 shadow-lg space-x-1">
                {interiorPositions.map((pos) => (
                  <button key={pos.id} onClick={() => setInteriorPosition(pos.id)} className={`px-3 py-1.5 md:px-5 md:py-2 rounded-full text-[9px] md:text-[11px] font-bold transition-all ${interiorPosition === pos.id ? 'bg-black text-white' : 'text-black hover:text-gray-600'}`}>
                    {pos.name}
                  </button>
                ))}
              </div>
